...
export default function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <aside
      className={`absolute inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 bg-gradient-to-b from-gray-900 via-gray-900 to-gray-950 text-white lg:static lg:translate-x-0 ${
        sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-0'
      }`}
    >
      <div className="flex items-center justify-between h-16 px-6 border-b border-gray-700/50 bg-gradient-to-r from-yellow-500/10 to-transparent">
        <span className="text-xl font-bold tracking-wider text-yellow-400">Gold Fleet</span>
        <button
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3">
        <ul className="space-y-2">
          {navigationSections.map((section, idx) => (
            <li key={idx}>
              <span className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-widest block">
                {section.title}
              </span>
      ...
      <div className="px-3 py-4 border-t border-gray-700/50">
        <div className="flex items-center gap-3 px-4 py-3 rounded-lg bg-gray-800/50">
          <div className="w-10 h-10 rounded-full bg-yellow-500/20 flex items-center justify-center text-yellow-400 text-sm font-semibold">
            GF
          </div>
          <span className="text-sm font-medium text-gray-300">Gold Fleet</span>
        </div>
      </div>
    </aside>
  );
